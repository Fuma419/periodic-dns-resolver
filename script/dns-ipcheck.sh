...
do
   : 
    ...
    # New setup likely: complete setup with "sudo ./dns-ipcheck.sh -f"
        ...
        $SCRIPT_PATH/tellegram_allert.sh "$NODE_ID Alert!%0A$i%0Ahas no ufw rule:%0AConsider adding rule with:%0A<sudo ./dns-ipcheck.sh -f>%0A$(date '+%B %d %Y %r')"
    ...
    # Could not resolve DNS for some reason. Likely a connction issue.
        ...
        $SCRIPT_PATH/tellegram_allert.sh "$NODE_ID Alert!%0A$i was unreachable at: %0A$old_ip %0AAwaiting connectivity. %0A$(date '+%B %d %Y %r')"
    ...
    # No matching firewall rule found. Lets add it now.
        ...
        $SCRIPT_PATH/tellegram_allert.sh "$NODE_ID Alert!%0A$i has no ufw rule:%0AAdding $new_ip ufw rule: %0A**Updating firewall** %0A$(date '+%B %d %Y %r')"
    ...
    # Detected IP address change. Taking action and/or notifiy operator.
        if [ $PASSIVE_MODE == "true" ] ; then
            $SCRIPT_PATH/tellegram_allert.sh "$NODE_ID Alert!%0AIP address change detected!%0A$i: %0APrevious IP:$old_ip%0ANew IP:$new_ip%0A %0AFirewall NOT updated! %0AVerify new IP then add ufw rule with <sudo ./dns-ipcheck.sh -f>: %0A$(date '+%B %d %Y %r')"
            ...
            $SCRIPT_PATH/tellegram_allert.sh "$NODE_ID Alert!%0AIP address change detected!%0A$i: %0APrevious IP:$old_ip%0ANew IP:$new_ip%0A %0A**Updating firewall now!** %0A$(date '+%B %d %Y %r')"
