...
DNS-ADDRESS[your-unique-node1-location]=https://www.w3.org/
#DNS-ADDRESS[your-unique-node2-location]=<your-unique-node2-dns-address>
#DNS-ADDRESS[your-unique-node3-location]=<your-unique-node3-dns-address>
#DNS-ADDRESS[your-unique-node4-location]=<your-unique-node4-dns-address>
...
do
   : 
    ...
    # No matching firewall rule found. Lets add it now.
        ...
        /usr/sbin/ufw allow proto tcp from $new_ip to any port $DNS_PORTS comment $i
    ...
    # Detected IP address change. Taking action and/or notifiy operator.
        if [ $PASSIVE_MODE == "true" ] ; then
            ...
            /usr/sbin/ufw delete allow proto tcp from $old_ip to any port $DNS_PORTS
            /usr/sbin/ufw allow proto tcp from $new_ip to any port $DNS_PORTS comment $i
